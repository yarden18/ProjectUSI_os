...
def frequency_distribution_word_counts_in_documents(df_dominant_topic, project_key, num_topics):
    ...
    if project_key!='DM':
        ...
        plt.gca().set(ylim=(0, 80))
    else:
        plt.text(600, 100, "Mean   : " + str(round(np.mean(doc_lens))))
        plt.text(600, 90, "Median : " + str(round(np.median(doc_lens))))
        plt.text(600, 80, "Stdev   : " + str(round(np.std(doc_lens))))
        plt.text(600, 70, "1%ile    : " + str(round(np.quantile(doc_lens, q=0.01))))
        plt.text(600, 60, "99%ile  : " + str(round(np.quantile(doc_lens, q=0.99))))
    ...
    plt.savefig('_/topic_model/distribution_issues_word_counts_{}.png'.format(project_key))
    plt.close()
    ...
    plt.savefig(
        '_/topic_model/distribution_issues_word_counts_byDominantTopic_{}.png'.format(
            project_key))
    plt.close()
...
def word_clouds_top_n_keywords_each_topic(stop_words, lda_model, project_key, num_topics):
    ...
    plt.tight_layout()
    plt.savefig(
        '_/topic_model/word_clouds_top_n_keywords_each_topic_{}.png'.format(
            project_key))
    plt.close()
...
def word_clouds_of_topic_keywords(lda_model, data_words_bigrams, project_key, num_topics):
    ...
    plt.savefig(
        '_/topic_model/word_clouds_of_topic_keywords_{}.png'.format(
            project_key))
    plt.close()
